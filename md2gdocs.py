...
def load_secrets() -> Dict[str, str]:
    ...
    if not secrets["client_id"] or not secrets["client_secret"]:
        logger.error(
            "Missing GOOGLE_CLIENT_ID or GOOGLE_CLIENT_SECRET. Set in .env or environment."
        )
        exit(1)
    return secrets
...
def upload_markdown_to_docs(
    ...
    file_name = Path(file_path).stem  # Use stem for document name
    doc_name = f"{file_name}"

    if template_doc_id:
        # Simplified approach: Use Google's conversion, then apply template elements
        # Step 1: Upload Markdown and let Google convert it (perfect formatting)
        markdown_file = (
            drive_service.files()
            .create(
                body={
                ...
                media_body=MediaFileUpload(
                    file_path, mimetype="text/markdown", resumable=True
                ),
            )
            .execute()
        )
        ...
        # Step 2: Convert to Google Docs format (Google does perfect conversion)
        converted_doc = (
            drive_service.files()
            .copy(
        ...
        logger.info(
            f"Converted Markdown to Google Doc with perfect formatting: {final_doc_id}"
        )

        # Step 3: Apply template styling by copying key elements
        # For now, we'll just use the converted document as-is
        # In a future enhancement, we could copy headers/footers from template
        logger.info(f"Template styling would be applied here (future enhancement)")

        # Step 4: Clean up temporary Markdown file
        ...
        # No template: Use the original approach with Google's conversion
        # Upload Markdown and let Google convert it
        markdown_file = (
            drive_service.files()
            .create(
                body={
...
def create_document(
    ...
    return upload_markdown_to_docs(
        drive_service, docs_service, md_file, template_doc_id, output_folder_id
    )
...
def convert(
    files: List[str] = typer.Argument(
        ..., help="Markdown files or glob patterns (e.g., *.md)"
    ),
    ...
    try:
        ...
        for md_file in md_files:
            ...
            logger.info(
                f"Created doc for {md_file}: https://docs.google.com/document/d/{doc_id}/edit"
            )
            typer.echo(
                f"Created document for {md_file}: https://docs.google.com/document/d/{doc_id}/edit"
            )
